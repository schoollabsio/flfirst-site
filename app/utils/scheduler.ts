...
const atMidnight = (now: Dayjs) =>
  now.hour() === 0 && now.minute() === 0 && onSecond(0)(now);
const atNoon = (now: Dayjs) =>
  now.hour() === 12 && now.minute() === 0 && onSecond(0)(now);
const atHour = (hour: number) => (now: Dayjs) =>
  now.hour() === hour && onMinute(0)(now) && onSecond(0)(now);
...
export const Monthly = {
  onFirstDay: (now: Dayjs) => now.date() === 1,
  onLastDay: (now: Dayjs) => now.date() === now.daysInMonth(),
  onDay: (day: number) => (now: Dayjs) => now.date() === day,
};
